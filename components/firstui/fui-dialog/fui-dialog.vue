...
<template>
  <view
    ...
    class="fui-dialog__wrap"
    :class="{ 'fui-wrap__show': show }"
  ...
  >
    <view
      ...
      @tap.stop="stop"
    >
      <text class="fui-dialog__title" :style="{ color: color }" v-if="title">{{
        title
      }}</text>
      ...
      <view class="fui-dialog__footer">
        <text
          ...
          class="fui-dialog__btn"
          :class="{ 'fui-dialog__btn-first': index === 0 }"
...
// #endif
const primaryColor =
  (uni && uni.$fui && uni.$fui.color && uni.$fui.color.primary) || "#465CFF";
export default {
  name: "fui-dialog",
  emits: ["click", "close"],
  props: {
