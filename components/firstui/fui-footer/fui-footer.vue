...
<template>
  <view
    class="fui-footer"
    :class="[isFixed ? 'fui-footer__fixed-bottom' : '']"
  ...
  >
    <view class="fui-footer__link" v-if="navigate.length > 0">
      <navigator
        ...
        :class="{ 'fui-link__color': !item.color }"
        ...
        :delta="item.delta"
        ><text
          class="fui-link__text"
          :class="{ 'fui-link__text-border': index === navigate.length - 1 }"
          :style="{
            color: item.color || linkColor,
    ...
    <view
      class="fui-footer__text"
      :class="{ 'fui-as__safe-weex': iphoneX && safeArea }"
    >
