...
<template>
  <view class="fui-bubble__box-wrap" ref="fui_bb_wrap">
    ...
    <view
      ...
      class="fui-bubble__box-mask"
      :class="[
        show ? 'fui-bb__unnvue-show' : '',
        isShow ? 'fui-bb__opciaty-show' : 'fui-bb__opciaty-hidden',
      ]"
      :style="getMaskStyl"
    ...
    <view
      ...
      :class="[
        direction === 'top' || direction === 'bottom'
          ? 'fui-bubble__box-col'
          : 'fui-bubble__box-row',
        position === 'absolute' ? 'fui-bb__list-' + direction : '',
        show ? 'fui-bb__unnvue-show' : '',
        isShow ? 'fui-bb__opciaty-show' : 'fui-bb__opciaty-hidden',
      ]"
      v-if="isVisible"
    >
      <view
        class="fui-bb__triangle-wrap"
        :style="getStyle"
        v-if="direction === 'bottom' || direction === 'right'"
      >
        <view
          class="fui-bubble__box-triangle"
          :style="getStyles"
          :class="['fui-bb__triangle-' + direction]"
        >
        </view>
      </view>
      <view
        class="fui-bubble__box-list"
        :style="{ background: background, width: width + 'rpx' }"
      >
        <view
          class="fui-bubble__box-item"
          :style="{
        ...
        >
          ...
          ></image>
          <text
            :class="{ 'fui-bubble__box-text': item.name || item.src }"
            :style="{
              fontSize: size + 'rpx',
              color: color,
          ...
          >
          <view
            class="fui-bubble__box-line"
            :class="[
              direction === 'left' ? 'fui-bb__line-right' : 'fui-bb__line-left',
            ]"
            :style="{
...
export default {
  ...
  computed: {
    ...
    getBbStyl() {
      ...
      if (this.position === "fixed") {
        ...
        switch (this.direction) {
          case "top":
            styl += `left:${this.n_width / 2 + this.n_left}px;top:${
              this.n_top + 2
            }px;`;
            break;
          case "left":
            styl += `left:${this.n_left + 2}px;top:${
              this.n_height / 2 + this.n_top
            }px;`;
            break;
          case "right":
            styl += `left:${this.n_left + this.n_width - 2}px;top:${
              this.n_height / 2 + this.n_top
            }px;`;
            break;
          case "bottom":
            styl += `left:${this.n_width / 2 + this.n_left}px;top:${
              this.n_top + this.n_height - 2
            }px;`;
            break;
          default:
            break;
    ...
    getStyles() {
      ...
      if (this.direction === "top" || this.direction === "bottom") {
        styl += $this.left
          ? `left:${Math.max(32, $this.left || 0)}rpx;`
          : `right:${Math.max(32, $this.right || 0)}rpx;`;
      } else {
        styl += $this.top
          ? `top:${Math.max(32, $this.top || 0)}rpx;`
