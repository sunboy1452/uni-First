...
<template>
  <view class="fui-uploadv__wrap">
    <view
      class="fui-uploadv__item"
      :style="{ width: width + 'rpx', height: height + 'rpx' }"
      v-for="(item, index) in urls"
      :key="index"
    >
      <video
        class="fui-upload__video"
        :style="{
          width: width + 'rpx',
      ...
      >
        <view class="fui-uploadv__flex-row" @tap.stop="reUpload(index)">
          ...
          ></fui-icon>
          <fui-icon
            :name="progress[index] === -99 ? 'clear' : 'wait'"
            color="#fff"
            :size="32"
            v-if="progress[index] < 0"
          ></fui-icon>
          <text
            class="fui-uploadv__progress-text"
            :class="{
              'fui-upt__pl': progress[index] === 100 || progress[index] < 0,
            }"
            >{{ getStatusText(progress[index]) }}</text
          >
          <text
            class="fui-uploadv__progress-text"
            v-if="progress[index] >= 0 && progress[index] < 100"
            >{{ progress[index] }}%</text
          >
        </view>
