...
<template>
  <view class="fui-pagination__wrap">
    <view
      class="fui-pagination__btn"
      :class="{
        'fui-pagination__disabled': currentIndex === 1,
        'fui-pagination__btn-ac': currentIndex !== 1 && highlight,
        'fui-pagination__color': !color,
      }"
      :style="{
        width: width + 'rpx',
        height: height + 'rpx',
    ...
    >
      <text
        :class="{ 'fui-pagination__color': !color }"
    ...
    <view class="fui-pagination__num" v-if="isPage && pageType == 1">
      <text
        :class="{ 'fui-pagination__active-color': !currentColor }"
        :style="{ color: getCurrentColor, fontSize: pageFontSize + 'rpx' }"
        >{{ currentIndex }}</text
      >
      <text
        :class="{ 'fui-pagination__color': !pageColor }"
        :style="{ color: pageColor, fontSize: pageFontSize + 'rpx' }"
        >/{{ maxPage || 0 }}</text
      >
    </view>
    ...
    <view class="fui-page__number" v-if="isPage && pageType == 2">
      <view
        class="fui-page__num-item"
        :class="{
          'fui-pagination__bg': !activeBgColor && currentIndex === item,
        }"
        v-for="(item, index) in pageNumber"
        :key="index"
        :style="{
          background: currentIndex === item ? getActiveBgColor : pageBgColor,
          borderRadius: radius + 'rpx',
        }"
        @tap.stop="handleClick(item, index)"
      >
        <text
          class="fui-page__num-text"
          :class="{ 'fui-pagination__color': !color && currentIndex !== item }"
    ...
    <view
      class="fui-pagination__btn"
      :class="{
        'fui-pagination__disabled': currentIndex === maxPage,
        'fui-pagination__btn-ac': currentIndex !== maxPage && highlight,
        'fui-pagination__color': !color,
      }"
      :style="{
        width: width + 'rpx',
