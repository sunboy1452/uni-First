...
<template>
  <view
    :id="elId"
    ref="fui_wi__el"
    class="fui-waterfall__item-wrap"
    :class="{ 'fui-waterfall__item-show': isShow }"
    :style="{
      width: width + 'px',
  ...
  >
    ...
    <view
      :class="{
        'fui-waterfall__img-wrap': !isLoaded,
        'fui-waterfall__hidden': imgHeight != 0,
      }"
      :style="getStyl"
      v-if="isSrc"
    >
      <image
        class="fui-waterfall__img"
        :src="src"
        :mode="imgHeight != 0 ? 'scaleToFill' : 'widthFix'"
        :webp="webp"
        :draggable="draggable"
        @load="handleLoad"
        @error="handleError"
...
export default {
  ...
  computed: {
    getStyles() {
      const width =
        this.imgWidth != 0 ? `${this.imgWidth}rpx` : `${this.width}px`;
    ...
    getWaterfallItemInfo() {
      this.getItemHeight((res) => {
        if (this.waterfall) {
          ...
          if (
            this.waterfall.childrenArr.length ===
            this.waterfall.loadedArr.length
          ) {
