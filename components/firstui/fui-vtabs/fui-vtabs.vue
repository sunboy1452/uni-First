  ...
  <view class="fui-vtabs__wrap" :style="{ width: vtabsW, height: vtabsH }">
    <view :style="{ width: tabWidth + 'rpx', background: background }">
      <scroll-view
        ...
        class="fui-vtabs__scroll-bar"
        :style="{ width: tabWidth + 'rpx' }"
        scroll-y
        :scroll-into-view="scrollInto"
        :scroll-with-animation="isTap"
      >
        <view
          class="fui-vtabs__item__wrap"
          :style="{ width: tabWidth + 'rpx' }"
        >
          <view
            class="fui-vtabs__item"
            :class="{
              'fui-vtabs__left-border': isBorder,
              'fui-vtabs__item-hover': !item.disable,
              'fui-vtabs__item-disable': item.disable,
              'fui-vtabs__border-color':
                !getBorderColor && current === index && isBorder,
            }"
            :id="'fui_vtabs_bar_' + index"
            v-for="(item, index) in vals"
            :key="index"
            :style="{
              background: current === index ? activeBgColor : background,
              borderLeftColor:
                current === index && isBorder ? getBorderColor : 'transparent',
              width: tabWidth + 'rpx',
          ...
          >
            ...
            <!-- #ifdef APP-NVUE -->
            <text
              class="fui-vtabs__text"
              :class="{
                'fui-vtabs__selected-color':
                  !getActiveColor && current === index,
              }"
              :style="{
              ...
              >{{ item.name
                ...
                :class="{
                  'fui-vtabs__badge-color': !getBadgeBackground,
                  'fui-vtabs__badge-dot': isDot,
                  'fui-vtabs__badge': !isDot,
                }"
  ...
  methods: {
    ...
    contentScroll(e) {
      ...
      if (scrollTop >= _heightRecords[0]) {
        for (let i = 1; i < len; i++) {
          if (
            scrollTop >= _heightRecords[i - 1] &&
            scrollTop < _heightRecords[i]
          ) {
            index = i;
            break;
          }
