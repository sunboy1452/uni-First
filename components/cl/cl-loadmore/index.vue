...
<template>
  <view
    class="fui-loadmore__wrap"
    :class="['fui-loadmore__' + direction]"
    :style="{ height: height + 'rpx' }"
  >
    <template v-if="status == 2">
      <view
        class="fui-loadmore__icon"
        ref="fui_loadmore"
    ...
    <text
      :class="{ 'fui-loadmore__text': direction === 'col' }"
      :style="{
        color: color,
        'font-size': size + 'rpx',
      ...
      >{{
        status == 2 ? text : status == 1 ? "没有更多啦~" : "没有更多啦~"
      }}</text
    >
  </view>
</template>
