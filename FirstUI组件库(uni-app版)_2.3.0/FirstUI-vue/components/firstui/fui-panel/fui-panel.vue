<template>
  <view
    ...
    :class="{
      'fui-panel__unlined': !isBorder,
      'fui-panel__background': !background,
    }"
  >
    <view
      class="fui-panel__hd"
    ...
    >
      <text
        :class="{ 'fui-panel__title-color': !headColor }"
        :style="{ fontSize: headSize + 'rpx', color: headColor + 'rpx' }"
        >{{ panelData[head] || "" }}</text
      >
      <view
        v-if="hdBorder"
        ...
        class="fui-panel__border"
        :class="{ 'fui-panel__border-color': !borderColor }"
      ></view>
    </view>
    <view
      class="fui-panel__mediabox"
      :style="{ paddingLeft: padding + 'rpx', paddingRight: padding + 'rpx' }"
      :class="{
        'fui-mediabox__center': !flexStart,
        'fui-panel__hover-weex': highlight,
      }"
      v-for="(item, index) in listData"
      :key="index"
      :hover-class="highlight ? 'fui-panel__hover' : ''"
      :hover-stay-time="150"
      @tap="handleClick(index)"
    >
      <view
        class="fui-panel__mb-hd"
        v-if="item[src]"
        :style="{ width: width + 'rpx', height: height + 'rpx' }"
      >
        <image
          class="fui-panel__mb-thumb"
          :src="item[src]"
          mode="widthFix"
          :style="{
            width: width + 'rpx',
      ...
      <view class="fui-panel__mb-bd">
        <text
          class="fui-panel__mb-title"
          v-if="item[title]"
          :class="{ 'fui-panel__title-color': !color }"
          :style="{
            color: color,
            fontSize: size + 'rpx',
        ...
        >
        <text
          class="fui-panel__mb-desc"
          v-if="item[desc]"
          :class="{ 'fui-panel__sub-color': !descColor }"
          :style="{ color: descColor, fontSize: descSize + 'rpx' }"
          >{{ item[desc] || "" }}</text
        >
        <view
          class="fui-panel__mb-info"
          v-if="item[source] || item[time] || item[extra]"
        >
          <text
            class="fui-panel__info-meta"
            :class="{ 'fui-panel__info-color': !infoColor }"
          ...
          >
          <text
            class="fui-panel__info-extra"
            :class="{
              'fui-panel__info-color': !infoColor,
              'fui-panel__extra-bcolor': !infoColor,
            }"
            :style="{
              color: infoColor,
