<template>
  ...
  >
    ...
    <text
      class="fui-result__title"
      :class="{ 'fui-result__title-color': !color }"
      :style="{ fontSize: size + 'rpx', color: color }"
      v-if="title"
      >{{ title }}</text
    >
    <text
      class="fui-result__descr"
      :class="{ 'fui-result__descr-color': !descrColor }"
