<template>
  <view class="fui-lottie__wrap">
    ...
    <view
      ...
      id="lottieRef"
      :prop="options"
      :change:prop="handler.changeProp"
      :action="actionVal"
      :change:action="handler.changeAction"
      :style="{ width: width + 'rpx', height: height + 'rpx' }"
    ></view>
    <!-- #endif -->
    <!-- #ifdef MP -->
    <canvas
      :style="{ width: lottieW + 'px', height: lottieH + 'px' }"
      :class="canvasId"
      type="2d"
    ></canvas>
    <!-- #endif -->
        ...
        query
          ...
          .node((res) => {
            ...
            this.aniMp = lottieMp.loadAnimation({
              ...
              autoplay:
                options.autoplay === undefined ? true : options.autoplay,
              //只支持网络地址
              path: options.path,
