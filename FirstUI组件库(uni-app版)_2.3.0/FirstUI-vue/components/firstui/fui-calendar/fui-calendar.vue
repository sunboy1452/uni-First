<template>
  ...
  >
    ...
    <view
      class="fui-calendar__date-header"
      ...
      <swiper-item v-for="(item, index) in monthArr" :key="item.key">
        <view class="fui-calendar__date-wrap">
          ...
          >
            ...
            >
            <view
              class="fui-calendar__badge"
              :style="{
                background: badgeColor || dangerColor,
                right: badgeRight + 'rpx',
                top: badgeTop + 'rpx',
              }"
              v-if="badgeArr[idx]"
            >
    ...
    <view class="fui-calendar__btn-wrap" v-if="showBtn">
      ...
      >
        <text
          class="fui-calendar__btn"
          :class="{
            'fui-calendar__btn-text': true,
            'fui-calendar__btn-active': !btnDisabled,
          }"
...
export default {
  ...
  computed: {
    ...
    itemWidth() {
      // #ifdef APP-NVUE
      return (
        (Number(this.dateWidth.replace("px", "").replace("%", "")) - 1) / 7 +
        "px"
      );
      // #endif
  ...
  methods: {
    ...
    _isRadius(index, idx, start) {
      ...
      if (this.type == 3 && this.values && this.values.length > 0) {
        let month = index + 1;
        let day = idx + 1;
        let date = `${this.year}-${this.formatNum(month)}-${this.formatNum(
          day
        )}`;
        if (start) {
      ...
      if (!this._isDisAbled(index, day - 1)) {
        ...
        if (type == 3) {
          ...
          if (start && values.length === 1 && sts <= ets) {
            ...
            if (
              (maxDays && maxDays > 0 && diffDays > maxDays) ||
              maxDays === 0
            ) {
              uni.showToast({
          ...
          if (idx != -1) {
            ...
            const maxDays = Number(this.maxDays);
            if (
              (maxDays && maxDays > 0 && values.length >= maxDays) ||
              maxDays === 0
            ) {
              uni.showToast({
