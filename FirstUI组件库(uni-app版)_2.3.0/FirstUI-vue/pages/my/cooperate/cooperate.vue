<template>
  <view class="fui-wrap">
    <view class="fui-banner__box">
      <image
        :src="`${resUrl}/cooperate/light/img_banner_3x.png`"
        mode="widthFix"
        class="fui-banner"
      ></image>
    </view>
      ...
      >
        <view class="fui-item__inner" :class="['fui-bg__' + idx]">
          <image
            ...
            :src="`/static/images/cooperate/light/icon_${model.icon}_3x.png`"
          >
          </image>
    ...
    <view class="fui-page__bd">
      <view class="fui-contacts__box">
        ...
        >
          <image
            class="fui-icon"
            :src="`/static/images/cooperate/light/icon_${item.icon}_3x.png`"
          ></image>
    ...
    >
      <view class="fui-ani__box">
        <view class="fui-ani__title">
          <image
            src="/static/images/cooperate/light/img_decorate_Left_3x.png"
            mode="widthFix"
          ></image>
          <text>{{ title }}</text>
          <image
            src="/static/images/cooperate/light/img_decorate_right_3x.png"
            mode="widthFix"
          ></image>
        </view>
        <view class="fui-flex__center fui-flex__column">
          ...
          <view class="fui-desc fui-p" v-if="idx === 4">
            在以任何形式的参与前，请先阅读文档开发指南。如有任何的意见或建议，欢迎您通过创建<text
              class="fui-link"
              @tap="copy($event, 'https://github.com/FirstUI/FirstUI/issues')"
              >Issue</text
            >或<text
              class="fui-link"
              @tap="copy($event, 'https://github.com/FirstUI/FirstUI/pulls')"
              >Pull Request</text
            >的方式告知我们。
          </view>
          <view class="fui-desc fui-p" v-if="idx === 5">
            First UI
            拥有成熟优秀的开发工程师团队及专业的UI设计师团队。产品研发团队中拥有多位行业经验丰富技术工程师。如需合作请通过页面联系方式与我们联系。
          </view>
          <fui-button
            btn-size="medium"
            radius="100rpx"
...
.fui-item__inner {
  ...
  font-size: 28rpx;
  color: #333333;
  background-color: rgba(254, 184, 8, 0.05);
  border-radius: 24rpx;
  /* #ifdef H5 */
  cursor: pointer;
  /* #endif */
...
.fui-bg__1 {
  background-color: rgba(9, 190, 79, 0.05);
}

.fui-bg__2 {
  background-color: rgba(70, 92, 255, 0.05);
}

.fui-bg__3 {
  background-color: rgba(104, 49, 255, 0.05);
}
