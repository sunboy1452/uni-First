<template>
  <view class="fui-wrap">
    <view class="fui-page__hd">
      <view class="fui-page__title fui-align__center">模板库 </view>
      <view class="fui-page__desc">常用模板，提升开发效率，付费模块。</view>
    </view>
    <view class="fui-page-bd fui-page__spacing">
      <view
        class="fui-template__item"
        @tap="href(item.page)"
        v-for="(item, index) in templateData"
        :key="index"
      >
        <view
          class="fui-template__item-inner"
          :style="{ background: item.background }"
        >
          <image
            class="fui-icon"
            :src="`${resUrl}/template/light/icon_${item.src}_3x.png`"
            mode="widthFix"
          ></image>
