<template>
  <view class="fui-wrap fui-page__bd">
    <view class="fui-banner__box">
      <image
        class="fui-banner"
        :src="`${resUrl}/my/light/img_banner_3x.png`"
        mode="widthFix"
      ></image>
      <view class="fui-vip__box fui-page__spacing">
        <view class="fui-mod__box">
          <image
            class="fui-vip__bg"
            :src="`${resUrl}/my/light/img_mod_bg.png`"
          ></image>
        </view>
        <view
          class="fui-vip__btn fui-flex__center"
          @tap="href('/pages/my/vip/vip')"
        >
    ...
    <view class="fui-page__spacing">
      <view class="fui-menu__wrap fui-align__center">
        <view
          ...
          :key="index"
          @click="href(item.page, $event)"
        >
          <button
            open-type="feedback"
            class="fui-btn__feedback"
            v-if="index === 2"
          ></button>
          <view
            class="fui-menu__img fui-flex__center"
            :style="{ background: item.background }"
          >
            <image
              :src="`${resUrl}/my/light/icon_${item.src}_3x.png`"
              mode="widthFix"
            ></image>
          </view>
          <text>{{ item.text }}</text>
      ...
      >
        <view class="fui-align__center">
          <image
            ...
            :src="`/static/images/my/light/icon_${item.src}_3x.png`"
            mode="widthFix"
          ></image>
          <text class="fui-list__text" style="margin-right: -20rpx">{{
            item.text
          }}</text>
        </view>
      </fui-list-cell>
  ...
  methods: {
    ...
    href(page, e) {
      if (page === "appreciate") {
        ...
        this.fui.modal(
          "意见反馈",
          "在以任何形式的参与前，请先阅读文档开发指南。如有任何的意见或建议，欢迎您通过创建 Issue 的方式告知我们。",
          (res) => {
            if (res) {
