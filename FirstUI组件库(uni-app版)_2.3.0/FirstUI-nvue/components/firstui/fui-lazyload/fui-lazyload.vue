<template>
  <view
    @appear="handleAppear"
    class="fui-lazyload__wrap"
    :style="{
      background: background,
      width: width == -1 ? '100%' : width + 'rpx',
      height: height == 0 || height == -1 ? 'auto' : height + 'rpx',
  ...
  >
    <image
      class="fui-lazyload__img"
      lazy-load
      :class="[visible ? 'fui-lazyload__visible' : 'fui-lazyload__hidden']"
      :style="getStyl"
      :src="show ? src : placeholder"
      :mode="height == 0 || height == -1 ? 'widthFix' : mode"
      :webp="webp"
      :draggable="draggable"
      @load="handleLoad"
      @error="handleError"
