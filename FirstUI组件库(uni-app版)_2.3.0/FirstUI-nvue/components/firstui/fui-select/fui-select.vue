<template>
  <view v-if="isShow || !isNvue">
    ...
    <view
      class="fui-select__mask"
      :class="{ 'fui-select__mask-show': show }"
      :style="getStyles"
      @tap.stop="maskClose"
      ref="fui_select_mk_ani"
    ></view>
    <view
      class="fui-select__wrap"
      ref="fui_select_ani"
      :class="{
        'fui-select__wrap-show': show,
        'fui-select__weex-android': isAndroid,
        'fui-select__safe-weex': iphoneX && safeArea,
      }"
      :style="{
    ...
    >
      ...
      >
        <view class="fui-select__list">
          ...
          >
            <view
              class="fui-select__item"
              :class="{
                'fui-select__reverse': isReverse && type === 'select',
                'fui-sitem__disable': model.disabled,
              }"
            >
              <view
                class="fui-select__checkbox"
                :class="{
                  'fui-select__is-checkmark ': isCheckMark,
                  'fui-select__checkbox-color':
                    (!checkboxColor || checkboxColor === true) &&
                    model.checked &&
                    !isCheckMark,
                }"
                :style="{
                  background:
                    model.checked && !isCheckMark
                      ? getCheckboxColor
                      : 'transparent',
                  borderColor:
                    model.checked && !isCheckMark
              ...
              <view class="fui-select__flex">
                <view
                  class="fui-select__icon-box"
                  :class="{
                    'fui-select__icon-ml': !isReverse && type === 'select',
                    'fui-select__icon-mr': isReverse,
                  }"
                  :style="{
                    width: iconWidth + 'rpx',
                ...
                <text
                  class="fui-select__item-text"
                  :class="{
                    'fui-select__text-pl':
                      !isReverse && (type === 'select' || model[srcKey]),
                    'fui-select__text-pr':
                      isReverse && (type === 'select' || model[srcKey]),
                  }"
      ...
      <view class="fui-select__btn-wrap" v-if="type === 'select'">
        <view
          class="fui-select__btn"
          :class="{ 'fui-select__btn-color': !btnBackground }"
          :style="{ background: getBgColor }"
        >
          <text
            class="fui-select__btn"
            :class="['fui-select__btn-text']"
            :style="{ color: btnColor }"
            @tap.stop="handleClick(false)"
            >{{ btnText }}</text
          >
        </view>
      </view>
...
export default {
  ...
  computed: {
    getStyles() {
      return `background:${this.maskBackground};z-index:${
  ...
  methods: {
    ...
    _animation(type) {
      if (!this.$refs["fui_select_ani"] || !this.$refs["fui_select_mk_ani"])
        return;
