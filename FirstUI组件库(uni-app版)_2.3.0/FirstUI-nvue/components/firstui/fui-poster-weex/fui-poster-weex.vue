  ...
  methods: {
    ...
    _drawText(context, params) {
      ...
      if (frontText) {
        ...
        x =
          this._getTextWidth(context, frontText, frontSize) +
          this._toPx(x + spacing);
        context.setFontSize(this._toPx(fontSize) * ratio);
      } else {
        x = this._toPx(x);
      }
      textArr.forEach((item, index) => {
        context.fillText(
          item,
          x,
          this._toPx(y + (lineHeight || fontSize) * index)
        );
      });
      context.restore();
      if (lineThrough) {
    ...
    _judgePermissionPhotoLibrary: async function (callback) {
      ...
      if (isAndroid) {
        result = await this._requestAndroidPermission(
        ...
        if (funcArr.length > 0) {
          Promise.all(funcArr)
            .then((res) => {
              ...
              const queue = this._getPosterData(
                texts,
                blocks,
                lines,
                imgs,
