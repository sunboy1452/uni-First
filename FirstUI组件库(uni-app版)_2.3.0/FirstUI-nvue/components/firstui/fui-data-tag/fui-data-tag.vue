<template>
  <view
    class="fui-data__tag-wrap"
    :class="[nowrap ? 'fui-data__tag-nowrap' : 'fui-data__tag-flexwrap']"
    :style="{ marginBottom: '-' + gap + 'rpx' }"
  >
    <!-- #ifndef APP-NVUE -->
    <view
      class="fui-data__tag-item"
      :class="{ 'fui-data__tag-disable': item.disable }"
      :style="{
    ...
    >
      ...
      <text
        class="fui-data__tag-text"
        :style="{
          fontSize: size + 'rpx',
          color: item.selected ? activeColor || primaryColor : color,
        }"
        >{{ item.text }}</text
      >
    </view>
...
export default {
  ...
  methods: {
    ...
    checkboxChange(index, model) {
      ...
      this.vals = vals;
      const entity = this.dataList.filter(
        (item) => vals.indexOf(item.value) != -1
      );
      let e = {
        detail: {
          value: vals,
