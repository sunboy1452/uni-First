<template>
  ...
  <text
    ...
    class="fui-icon"
    :class="[
      !getColor && !primary ? 'fui-icon__color' : '',
      primary && (!color || color === true) ? 'fui-icon__active-color' : '',
      disabled ? 'fui-icon__not-allowed' : '',
      customPrefix && customPrefix !== true ? customPrefix : '',
  ...
  <text
    ...
    class="fui-icon"
    :class="[customPrefix && customPrefix !== true ? customPrefix : '']"
    @click="handleClick"
    >{{ customPrefix && customPrefix !== true ? name : icons[name] }}</text
  >
  <!-- #endif -->
</template>
...
export default {
  ...
  computed: {
    getSize() {
      const size =
        (uni.$fui && uni.$fui.fuiIcon && uni.$fui.fuiIcon.size) || 64;
      const unit =
        (uni.$fui && uni.$fui.fuiIcon && uni.$fui.fuiIcon.unit) || "rpx";
