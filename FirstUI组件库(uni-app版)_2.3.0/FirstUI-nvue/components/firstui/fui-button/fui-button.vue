<template>
  <view
    ...
    :class="[
      !getWidth || getWidth === '100%' || getWidth === true
        ? 'fui-button__flex-1'
        : '',
  ...
  >
    <button
      ...
      :class="[
        ...
        !background && !disabledBackground && !plain
          ? 'fui-button__' + type
          : '',
        !getWidth || getWidth === '100%' || getWidth === true
          ? 'fui-button__flex-1'
      ...
      :style="{
        ...
        background: disabled
          ? disabledBackground || getTypeColor
          : plain
          ? 'transparent'
          : getBackground,
    ...
    >
      <text
        ...
        v-if="text"
        :style="{ fontSize: getSize, lineHeight: getSize, color: getColor }"
        >{{ text }}</text
      >
      <slot></slot>
    </button>
    ...
    <view
      v-if="borderColor"
      class="fui-button__thin-border"
      :class="[
        time && (plain || type === 'link') && !disabled
          ? 'fui-button__opacity'
          : '',
        disabled && !disabledBackground ? 'fui-button__opacity' : '',
      ]"
...
export default {
  ...
  props: {
    ...
    //V2.3.0+ 按钮大小，优先级高于width和height，medium、small、mini
    btnSize: {
      type: String,
      default: "",
    },
  ...
  computed: {
    ...
    getColor() {
      ...
      if (this.color) {
        color =
          this.disabled && this.disabledBackground
            ? this.disabledColor
            : this.color;
      } else {
    ...
    getSize() {
      let size =
        this.size ||
        (uni && uni.$fui && uni.$fui.fuiButton && uni.$fui.fuiButton.size) ||
        32;
    ...
    getHeight() {
      let height =
        this.height ||
        (uni && uni.$fui && uni.$fui.fuiButton && uni.$fui.fuiButton.height) ||
        "96rpx";
      if (this.btnSize && this.btnSize !== true) {
        height =
          {
    ...
    getBorderRadius() {
      let radius =
        (uni && uni.$fui && uni.$fui.fuiButton && uni.$fui.fuiButton.radius) ||
        "16rpx";
