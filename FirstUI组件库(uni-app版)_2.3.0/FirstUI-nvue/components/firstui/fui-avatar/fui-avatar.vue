<template>
  <view
    class="fui-avatar__wrap"
    :class="[
      width ? '' : 'fui-avatar__size-' + size,
      radius === -1 ? 'fui-avatar__' + shape : '',
    ]"
    :style="wrapStyles"
    @tap="handleClick"
  >
    ...
    ></image>
    <text
      class="fui-avatar__text"
      :class="[width ? '' : 'fui-avatar__text-' + size]"
      v-if="!src && src !== true && text"
      :style="textStyles"
      >{{ text }}</text
    >
    <slot></slot>
...
export default {
  ...
  computed: {
    styles() {
      let styles = "";
      if (this.width) {
        styles = `width:${this.width}rpx;height:${
