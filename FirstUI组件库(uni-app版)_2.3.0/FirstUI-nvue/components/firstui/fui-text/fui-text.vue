<template>
  <view
    class="fui-text__wrap"
    :class="[
      block ? 'fui-text__block' : 'fui-text__inline',
      'fui-text__' + align,
      highlight && !disable ? 'fui-text__active' : '',
    ]"
    :style="{
      textAlign: align,
  ...
  >
    ...
    <text
      ...
      :class="[
        color ? '' : 'fui-text__' + type,
        unShrink ? 'fui-text__unshrink' : '',
      ]"
...
export default {
  ...
  computed: {
    getSize() {
      const size =
        (uni && uni.$fui && uni.$fui.fuiText && uni.$fui.fuiText.size) || 32;
      const unit =
        (uni && uni.$fui && uni.$fui.fuiText && uni.$fui.fuiText.unit) || "rpx";
