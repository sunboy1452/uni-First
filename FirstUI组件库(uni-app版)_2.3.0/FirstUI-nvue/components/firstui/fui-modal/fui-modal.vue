<template>
  <view
    class="fui-modal__wrap"
    :style="{ background: maskBackground, zIndex: zIndex }"
    :class="{ 'fui-modal__fadein': isShow }"
    @tap="closeModal"
    v-if="isShow || !isNvue"
    ref="fui_modal_ani"
  >
    ...
    >
      <text
        class="fui-modal__title"
        :style="{ fontSize: size + 'rpx', color: color }"
        v-if="title && title !== true"
        >{{ title }}</text
      >
      <text
        class="fui-modal__descr"
        :style="{ fontSize: descrSize + 'rpx', color: descrColor }"
        :class="{ 'fui-modal__descr-pt': !title }"
        v-if="descr && descr !== true"
        >{{ descr }}</text
      >
      <slot></slot>
      <view
        class="fui-modal__btn-wrap"
        :style="getWidth"
        :class="{ 'fui-modal__btn-row': direction === 'row' }"
        v-if="vals.length > 0"
      >
        <view
          class="fui-modal__button"
          :class="{
            'fui-model__button-full': direction === 'row',
            'fui-modal__button-col':
              direction === 'column' && index !== vals.length - 1,
            'fui-modal__button-bg': !entity.plain && !entity.background,
            'fui-modal__button-border': entity.plain,
            'fui-modal__button-bc': entity.plain && !entity.background,
          }"
          :style="{
            borderRadius: radius + 'rpx',
            background: entity.plain
              ? 'transparent'
              : entity.background || primaryColor,
            borderColor: entity.plain
              ? entity.background || primaryColor
              : 'transparent',
          }"
        ...
        >
          <text
            class="fui-modal__button-inner"
            :class="{
              'fui-modal__button-color': !entity.color && entity.plain,
            }"
            :style="{
              color: entity.color || (entity.plain ? primaryColor : '#fff'),
              borderRadius: radius + 'rpx',
            }"
