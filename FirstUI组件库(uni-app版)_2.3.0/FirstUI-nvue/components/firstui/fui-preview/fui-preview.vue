<template>
  <view
    class="fui-preview__wrap"
    :class="{
      'fui-preview__unlined': !isBorder,
      'fui-preview__background': !background,
    }"
    :style="{
  ...
  >
    ...
    >
      <view class="fui-preview__item">
        <text
          ...
          :class="{ 'fui-preview__gray': !labelColor }"
          :style="{
            color: labelColor,
            fontSize: hdLabelSize + 'rpx',
            width: hdLabelWidth > 0 ? hdLabelWidth + 'rpx' : 'auto',
            marginRight: labelRight + 'rpx',
          }"
          >{{ pvd[label] || "" }}</text
        >
        <text
          class="fui-preview__value-hd"
          :class="{ 'fui-preview__color': !hdValColor }"
          :style="{
            color: hdValColor,
            fontSize: hdValSize + 'rpx',
            textAlign: bdAlign,
          }"
          >{{ pvd[value] || "" }}</text
        >
      ...
      <view
        ...
        class="fui-preview__border"
        :class="{ 'fui-preview__border-color': !borderColor }"
      ></view>
    </view>
    <view
      class="fui-preview__bd"
      v-if="pvd[list] && pvd[list].length > 0"
      :style="{ paddingLeft: padding + 'rpx', paddingRight: padding + 'rpx' }"
    >
      <view
        class="fui-preview__item"
        v-for="(item, index) in pvd[list]"
        :key="index"
      >
        <text
          class="fui-preview__label"
          :class="[
            !labelColor && !item[lColor] ? 'fui-preview__gray' : '',
            'fui-preview__label-' + labelAlign,
          ]"
          :style="{
        ...
        <!-- #ifndef APP-NVUE -->
        <text
          class="fui-preview__value"
          :class="{ 'fui-preview__color': !bdColor && !item[valueColor] }"
          :style="{
    ...
    >
      <view
        :style="{
          'border-right-color':
            idx !== pvd[buttons].length - 1 ? borderColor : 'transparent',
        }"
        :class="[
          highlight && !btn[disabled] ? 'fui-preview__hover-weex' : '',
          idx !== pvd[buttons].length - 1
            ? 'fui-preview__btn-def'
            : 'fui-preview__btn',
        ]"
        :hover-class="highlight && !btn[disabled] ? 'fui-preview_hover' : ''"
        :hover-stay-time="150"
      ...
      >
        <text
          ...
          :class="{
            'fui-preview__link': !btn[color],
            'fui-preview__button-disable': btn[disabled],
          }"
          >{{ btn[text] }}</text
        >
      </view>
