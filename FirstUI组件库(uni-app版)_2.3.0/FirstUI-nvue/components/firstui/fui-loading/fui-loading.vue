<template>
  <view
    :class="{ 'fui-loading__mask': (isMask || isNvue) && isFixed }"
    :style="{ backgroundColor: isFixed ? maskBgColor : 'transparent' }"
  >
    <view
      class="fui-loading fui-loading__wrap"
      :class="{ 'fui-loading__fixed': isFixed && !isNvue }"
      :style="{
    ...
    <view
      v-else
      class="fui-loading fui-loading__row"
      :class="{ 'fui-loading__fixed': isFixed && !isNvue }"
