    ...
    cbAni(e) {
      ...
      switch (this.direction) {
        ...
        case "rb":
          diff.x = this.width - touch.clientX - uni.upx2px(Number(this.right));
          diff.y =
            this.height - touch.clientY - uni.upx2px(Number(this.bottom));
          break;
        case "lb":
      ...
      //移动距离
      // #ifdef APP-NVUE
      this.style_x = `left:${touch.clientX}px;top:${
        touch.clientY - this.navH
      }px;`;
      // #endif
      this.animate = true;
      this.$nextTick(() => {
        setTimeout(() => {
          // #ifdef APP-NVUE
          this.style_x += `transform:translate(${diff.x}px,0px);`;
