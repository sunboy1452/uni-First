<template>
  <view
    class="fui-tabbar__wrap"
    :class="{ 'fui-tabbar__fixed': isFixed }"
    :style="{ background: background, zIndex: zIndex }"
  >
    <view
      class="fui-tabbar__inner"
      :class="{ 'fui-tababr__border': isBorder }"
      :style="{
        borderTopColor: isBorder && isNvue ? borderColor : 'transparent',
      }"
    >
      <view
        class="fui-tabbar__item"
        v-for="(item, index) in tabs"
        :key="index"
        @tap="itemClick(index)"
      >
        <view
          class="fui-tabbar__icon-wrap"
          :style="{
            width:
        ...
        >
          <image
            class="fui-tabbar__icon"
            :src="current == index ? item.selectedIconPath : item.iconPath"
            v-if="!item.midButton && item.iconPath"
          ></image>
          <!-- #ifdef APP-NVUE -->
          <image
            v-if="item.midButton"
          ...
          <!-- #endif -->
          <fui-icon
            v-if="item.iconName && !item.iconPath && !item.midButton"
            :name="current == index ? item.selectedIconName : item.iconName"
            :size="item.iconSize || iconSize"
            :unit="item.iconUnit || iconUnit"
            :color="
              current == index
                ? item.selectedColor || getSelectedColor
                : item.color || color
            "
          ...
          <text
            :class="[
              item.dot ? 'fui-tabbar__dot' : 'fui-tabbar__badge',
              badgeBackground ? '' : 'fui-tabbar__badge-color',
            ]"
            :style="{
        ...
        <text
          class="fui-tabbar__text"
          :class="{
            'fui-tabbar__selected-color': !getSelectedColor && current == index,
          }"
          :style="{
            fontSize: size + 'rpx',
        ...
        <image
          v-if="item.midButton"
          class="fui-tabbar__mid-button"
          :class="{ 'fui-tabbar__mid-center': item.center }"
          :src="item.iconPath"
          :style="{
            width: (item.width || 56) + 'rpx',
            height: (item.height || 56) + 'rpx',
    ...
    <view
      class="fui-tabbar__safe-area"
      :class="{ 'fui-tabbar__safe-weex': safeAreaH > 0 }"
      v-if="(safeAreaH > 0 || !isNvue) && safeArea && isFixed"
    ></view>
  </view>
</template>
