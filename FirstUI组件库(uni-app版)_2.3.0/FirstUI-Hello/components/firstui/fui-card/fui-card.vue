<template>
  <view
    class="fui-card__wrap"
    :class="{
      'fui-card__full': full,
      'fui-card__border': showBorder,
      'fui-card__border-radius': showBorder && !isNvue && !full,
    }"
    :style="{
      marginTop: margin[0] || 0,
      marginRight: full ? 0 : margin[1] || 0,
      marginBottom: margin[2] || margin[0] || 0,
      marginLeft: full ? 0 : margin[3] || margin[1] || 0,
      background: background,
      borderRadius: full ? 0 : radius,
      'box-shadow': showBorder ? 'none' : shadow,
  ...
  >
    <view
      ...
      v-if="tag || title || src"
      :class="{ 'fui-card__header-line': headerLine }"
      :style="{
    ...
    >
      ...
      <view class="fui-card__header-right" v-if="tag">
        <text :style="{ fontSize: tagSize + 'rpx', color: tagColor }">{{
          tag
        }}</text>
      </view>
    </view>
    <view
      class="fui-card__body"
      :class="{ 'fui-card__header-line': footerLine }"
    >
      <slot></slot>
    </view>
    <view
