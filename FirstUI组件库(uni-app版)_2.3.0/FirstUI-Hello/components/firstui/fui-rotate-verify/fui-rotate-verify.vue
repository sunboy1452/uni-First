<template>
  <view
    class="fui-rotate__verify-wrap"
    :class="{ 'fui-rv__wrap-show': isShow }"
  ...
  >
    <view
      class="fui-rotate__verify"
      :style="{ width: width + 'rpx', background: background }"
      @tap.stop="stop"
    >
      ...
      <text
        class="fui-rv__descr"
        :style="{ color: descrColor, fontSize: descrSize + 'rpx' }"
        >拖动滑块，使图片角度为正</text
      >
      <view
        class="fui-rv__image-box"
        :style="{
          width: imgW + 'px',
          height: imgW + 'px',
      ...
      >
        ...
        <image
          class="fui-rv__image"
          :class="{ 'fui-rv__rest-ani': resetAni }"
          :src="src"
          :style="{
            width: imgW + 'px',
            height: imgW + 'px',
...
export default {
  ...
  methods: {
    ...
    verifyRes(angle, initAngle, range, callback) {
      let res = false;
      if (
        Math.abs(angle + initAngle - 360) <= range ||
