<template>
  <view
    class="fui-alert__wrap"
    :class="[background ? '' : 'fui-alert__' + type]"
    :style="{
  ...
  >
    <view class="fui-alert__shrink" @tap.stop="leftClick">
      ...
      <icon
        ...
        :size="iconSize"
        :color="iconColor"
        v-if="!isLeft && type && type !== 'true'"
      ></icon>
    </view>
    <view
      class="fui-alert__content"
      :class="{
        'fui-text__p-left':
          (!isLeft && type && type !== 'true') || (spacing && isLeft),
        'fui-text__p-right': closable,
      }"
      @tap.stop="onClick"
    >
      <text
      ...
      >
      <text
        class="fui-alert__text fui-desc__padding"
        :class="{ 'fui-alert__single': single }"
