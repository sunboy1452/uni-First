<template>
  ...
  >
    <view class="fui-sections__title" @tap="handleClick">
      <view
        class="fui-sections__line"
        :class="{ 'fui-sections__line-color': !getLineColor }"
        :style="{
          background: getLineColor,
          width: lineWidth,
          top: lineGap + 'rpx',
          bottom: lineGap + 'rpx',
          borderRadius: lineCap === 'circle' ? lineWidth : 0,
          left: getLeft,
        }"
        v-if="isLine"
      >
    ...
    <view class="fui-sections__descr" :style="getDescrTop" v-if="descr">
      <text class="fui-sections__descr-text" :style="getDescrStyle">{{
        descr
      }}</text>
    </view>
...
export default {
  ...
  computed: {
    ...
    getTitleStyle() {
      ...
      const left = Number(this.lineRight || 0);
      let style = `font-size:${size}rpx;color:${color};font-weight:${weight};padding-left:${
        left <= 0 || !this.isLine ? 0 : left
      }rpx;`;
      style += `line-height:${
        this.lineHeight == 0 ? size : this.lineHeight
      }rpx`;
      return style;
    },
    getDescrStyle() {
    ...
    getDescrTop() {
      const app = uni && uni.$fui && uni.$fui.fuiSection;
      return (
        "padding-top:" + (this.descrTop || (app && app.descrTop) || 8) + "rpx;"
      );
    },
  },
  methods: {
