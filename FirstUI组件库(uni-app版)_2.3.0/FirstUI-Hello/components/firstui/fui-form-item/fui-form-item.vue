<template>
  ...
  >
    ...
    >
      ...
      >
        ...
        <view
          ...
          v-if="asterisk"
          :style="{
            color: asteriskColor || akColor || dangerColor,
            left: getAkPosi,
          }"
          >*</view
        >
    ...
    <view
      class="fui-form__item-error"
      :class="{
        'fui-form__error-absolute': getErrorPosition == 1,
        'fui-form__error-right': getErrorAlign == 'right',
        'fui-form__error-active': errorMsg && errorMsg !== true,
      }"
      v-if="getErrorPosition == 1 && showError && prop"
      :style="{
        paddingLeft: getErrorLeft,
        paddingRight: padding[1] || '32rpx',
      }"
    >
      <text
        class="fui-form__error-text"
        :class="{ 'fui-form__error-right': getErrorAlign == 'right' }"
        :style="{ color: asteriskColor || akColor || dangerColor }"
        >{{ errorMsg }}</text
      >
    </view>
    <view
      class="fui-form__item-error"
      :class="{
        'fui-form__error-relative': getErrorPosition == 2,
        'fui-form__error-right': getErrorAlign == 'right',
      }"
      v-if="
        getErrorPosition == 2 &&
...
export default {
  ...
  computed: {
    ...
    getLabelSize() {
      const labelSize =
        (uni.$fui && uni.$fui.fuiFormItem && uni.$fui.fuiFormItem.labelSize) ||
        32;
      return `${this.labelSize || this.lSize || labelSize}rpx`;
    },
    getLabelWidth() {
      const labelWidth =
        (uni.$fui && uni.$fui.fuiFormItem && uni.$fui.fuiFormItem.labelWidth) ||
        160;
      return `${this.labelWidth || this.lWidth || labelWidth}rpx`;
    },
    getLabelRight() {
      const labelRight =
        (uni.$fui && uni.$fui.fuiFormItem && uni.$fui.fuiFormItem.labelRight) ||
        30;
      return `${this.labelRight || labelRight}rpx`;
    },
    getLabelAlign() {
      const labelAlign =
        (uni.$fui && uni.$fui.fuiFormItem && uni.$fui.fuiFormItem.labelAlign) ||
        "left";
    ...
    getLabelWeight() {
      const global =
        (uni.$fui &&
          uni.$fui.fuiFormItem &&
          uni.$fui.fuiFormItem.labelWeight) ||
        400;
    ...
    getAkPosi() {
      const akPosi =
        (uni.$fui &&
          uni.$fui.fuiFormItem &&
          uni.$fui.fuiFormItem.asteriskPosition) ||
        "left";
  ...
  methods: {
    ...
    setRulesMerge(rules) {
      ...
      if (this.form) {
        const index = this.form.mergeRules.findIndex(
          (e) => e.name === rules.name || e.name === this.prop
        );
    ...
    validator(value) {
      ...
      return new Promise((resolve, reject) => {
        if (this.form && rules) {
          ...
          this.form
            .realTimeValidator(rules.name, model, [rules])
            .then((res) => {
              if (res.isPassed) {
                this.errorMsg = "";
              } else {
