<template>
  <view
    class="fui-input__wrap"
    :class="{ 'fui-input__border-nvue': inputBorder }"
    :style="{
  ...
  >
    ...
    <!-- #ifndef APP-NVUE -->
    <view
      class="fui-input__border"
      :class="{ 'fui-input__bordercolor': !borderColor }"
      :style="{ borderRadius: getBorderRadius, borderColor: borderColor }"
      v-if="inputBorder"
    ></view>
    <!-- #endif -->
    <!-- #ifdef APP-NVUE -->
    <view class="fui-input__required" v-if="required">
      <text
        :style="{ color: requiredColor || dangerColor }"
    ...
    <view
      ...
      v-if="label"
    >
      <text :style="{ fontSize: getLabelSize, color: labelColor }">{{
        label
      }}</text>
    </view>
    <slot name="left"></slot>
    <!-- #ifndef APP-NVUE -->
    <input
      class="fui-input__self"
      :class="{
        'fui-input__text-right': textRight,
        'fui-input__disabled-styl': disabled && disabledStyle,
        'fui-input__disabled': disabled,
      }"
      :style="{
        fontSize: getSize,
        color: color,
        textAlign: textRight ? 'right' : textAlign,
      }"
      ...
      :password="password || type === 'password' || null"
    ...
    <view class="fui-input__self-wrap">
      <input
        ref="fuiInput"
        class="fui-input__self"
        :class="{ 'fui-input__text-right': textRight }"
        :style="{
          fontSize: getSize,
          color: color,
          textAlign: textRight ? 'right' : textAlign,
...
export default {
  ...
  computed: {
    ...
    getSize() {
      const size =
        (uni.$fui && uni.$fui.fuiInput && uni.$fui.fuiInput.size) || 32;
      return `${this.size || size}rpx`;
    },
    getLabelSize() {
      const labelSize =
        (uni.$fui && uni.$fui.fuiInput && uni.$fui.fuiInput.labelSize) || 32;
  ...
  methods: {
    ...
    onInput(event) {
      ...
      if (
        (this.modelModifiers.number ||
          this.number ||
          this.type === "digit" ||
          this.type === "number") &&
        !isNaN(currentVal) &&
        Number.isSafeInteger(currentVal)
      ) {
