...
export default {
  ...
  methods: {
    ...
    emitSelectionChange() {
      const itemList = this.tableData.filter(
        (item) => item.is_selected === true && item.is_disabled !== true
      );
    ...
    checkSelectionAll() {
      ...
      const index = this.tableData.findIndex(
        (item) => item.is_selected === false && item.is_disabled !== true
      );
      if (~index) {
        this.chkAll = false;
      } else {
        ...
        if (asc) {
          ...
          if (item.sortType === "number") {
            ...
            this.tableData.sort((a, b) => {
              //日期格式字符串必须可以被转化为日期格式
              return (
                new Date(a[item.prop].replace(/\-/g, "/")).getTime() -
                new Date(b[item.prop].replace(/\-/g, "/")).getTime()
              );
            });
          } else {
            this.tableData.sort((a, b) => {
