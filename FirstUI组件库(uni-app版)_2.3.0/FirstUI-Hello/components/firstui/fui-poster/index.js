...
const poster = {
    ...
    _wrapText(text, fontSize, textWidth, width, context, rows = 2) {
        ...
        if (textWidth > width) {
            ...
            for (let i = 0, len = arr.length; i < len; i++) {
                ...
                if (
                    poster._getTextWidth(context, fillText, fontSize) >= width
                ) {
    ...
    _drawText(context, params) {
        ...
        if (frontText) {
            ...
            x =
                poster._getTextWidth(context, frontText, frontSize) +
                poster._toPx(x + spacing);
            context.setFontSize(poster._toPx(fontSize));
        } else {
            x = poster._toPx(x);
        }
        textArr.forEach((item, index) => {
            context.fillText(
                item,
                x,
                poster._toPx(y + (lineHeight || fontSize) * index)
            );
        });
        context.restore();
        if (lineThrough) {
    ...
    _drawQrcode(context, params) {
        ...
        cells.forEach((row, rdx) => {
            row.forEach((cell, cdx) => {
                ...
                context.fillRect(
                    Math.round(cdx * tileW) + x,
                    Math.round(rdx * tileH) + y,
                    w,
                    h
                );
            });
    ...
    _judgePermissionPhotoLibrary: async function (callback) {
        ...
        if (isAndroid) {
            result = await poster._requestAndroidPermission(
