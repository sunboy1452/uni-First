<template>
  <text
    class="fui-digitalnumbers"
    :class="{
      'fui-number__highlight': !disabled && highlight,
      'fui-number__color': !color || color === true,
    }"
    :style="{
      color: getColor,
      fontSize: getSize,
...
export default {
  ...
  computed: {
    getSize() {
      const size =
        (uni.$fui && uni.$fui.fuiNumber && uni.$fui.fuiNumber.size) || 32;
      const unit =
        (uni.$fui && uni.$fui.fuiNumber && uni.$fui.fuiNumber.unit) || "rpx";
